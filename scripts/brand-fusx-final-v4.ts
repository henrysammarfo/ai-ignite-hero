...
async function main() {
    ...
    console.log("New Branded Mint (V4 TS):", mint.publicKey.toBase58());
    ...
    // THE KEY: Calculating space for TokenMetadata manually 
    // Metadata structure: 
    // updateAuthority (32) + mint (32) + name (4+len) + symbol (4+len) + uri (4+len) + additional (4)
    const metadataSpace = 32 + 32 + (4 + name.length) + (4 + symbol.length) + (4 + uri.length) + 4;
    const mintLen = getMintLen(extensions) + metadataSpace;
    const lamports = await connection.getMinimumBalanceForRentExemption(mintLen);

    console.log("Total Space Allocated:", mintLen);
