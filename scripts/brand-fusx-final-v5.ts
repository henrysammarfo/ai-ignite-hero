...
async function main() {
    ...
    console.log("New Branded Mint (V5 Fix):", mint.publicKey.toBase58());
    ...
    // Extensions EXCLUDING TokenMetadata for getMintLen
    ...
    // getMintLen only works for fixed-size extensions
    const baseLen = getMintLen(baseExtensions);
    
    // Manual extra space for Metadata Extension + Content
    const metadataExtensionOverhead = 4; // Type (2) + Len (2)
    const metadataContentSize = 512; // Generous buffer for strings
    
    const totalLen = baseLen + metadataExtensionOverhead + metadataContentSize;
    const lamports = await connection.getMinimumBalanceForRentExemption(totalLen);

    console.log("Base Len:", baseLen);
    console.log("Total Space Allocated:", totalLen);
    ...
    try {
        ...
        console.log("Solscan/Explorer WILL show 'Fortis USX' and 'fUSX'.");
    } catch (err: any) {
