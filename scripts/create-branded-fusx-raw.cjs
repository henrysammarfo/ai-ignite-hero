...
async function main() {
    ...
    // Calculate space for metadata manually
    // Name (long) + Symbol (short) + URI (long)
    const baseMintLen = getMintLen(extensions);
    const space = baseMintLen + 512; 
    ...
    const tx = new Transaction().add(
        ...
        // This is the CRITICAL instruction that populates the metadata
        createInitializeInstruction({
            programId: TOKEN_2022_PROGRAM_ID,
